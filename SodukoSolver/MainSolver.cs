...
namespace SodukoSolver
{
    public class MainSolver
    {
        ...
        public bool FillRecursively(MainBackend matrix)
        {
            bool success = true;
        ...
        private bool SolveRecursevly(MainBackend matrix, int depth)
        {
            if (stopping)
                return false;
        ...
        public int FillOnes(MainBackend matrix)
        {
        ...
        public int EmptySquares(MainBackend matrix)
        {
            var nn = matrix.board.IterateBoard(square =>
            {
                if (square.Value == SodukoSet.EmptyValue)
                    return 1;
                else
